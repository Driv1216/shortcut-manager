...
function buildIndex(shortcuts) {
  const nextIndex = new Map();

  shortcuts.forEach((shortcut) => {
    // The runtime map only stores active shortcuts so lookup stays O(1)
    // and disabled items never trigger accidentally.
    if (shortcut.enabled && shortcut.normalizedKeys) {
      nextIndex.set(shortcut.normalizedKeys, shortcut);
    }
  });
...
export async function rebuildRuntimeIndex() {
  ...
  runtimeIndex = buildIndex(shortcuts);
  return runtimeIndex;
}
...
export function exportRuntimeIndexAsObject() {
  const obj = {};
  for (const [key, value] of runtimeIndex.entries()) {
    obj[key] = value;
  }
  return obj;
}
